    ...
    try:
        with sd.InputStream(samplerate=sample_rate, channels=1, dtype='int16') as stream:
            ...
            while True:
                ...
                if len(dataflow) == 0:
                    break
                result = ANS_pipeline(create_wav_header(dataflow, sample_rate=sample_rate, num_channels=1, bits_per_sample=16))
                output = result['output_pcm']
                vad_buffer = create_wav_header(output, sample_rate=sample_rate, num_channels=1, bits_per_sample=16)
                ...
                if(res[0]["value"]):
                    ...
                    if len(buffer) > 0:
                        vad_queue.put(create_wav_header(buffer, sample_rate=sample_rate, num_channels=1, bits_per_sample=16))
                        buffer = b''
