...
class SemanticMapGenerator:
    def __init__(self):
        ...
        self.current_prompt = rospy.get_param(
            "~default_prompt",
            "keyboard. mouse. cell phone. earphone. laptop. cup. keys. potted plant",
    ...
    #     # 检查输入点云长度是否一致
    #     if not (len(x) == len(y) == len(z) == len(rgb)):
    #         print("[错误] 输入点云的长度不一致")
    #         return [], [], [], []

    #     try:
    #         # 将点云数据转换为 numpy 数组
    #         points = np.column_stack((x, y, z)).astype(np.float64)  # 组合成 Nx3 数组
    #     except Exception as e:
    #         print(f"[错误] 创建点云数组失败: {e}")
    #         return [], [], [], []
    ...
    #         print(f"[信息] 统计滤波后点云数量: {len(pcd.points)}")
    #     except Exception as e:
    #         print(f"[错误] 统计离群点滤波失败: {e}")
    #         return [], [], [], []
    ...
    #             return [], [], [], []
    #         pcd = pcd.select_by_index(ind)
    #         print(f"[信息] 半径滤波后点云数量: {len(pcd.points)}")
    #     except Exception as e:
    #         print(f"[错误] 半径离群点滤波失败: {e}")
    #         return [], [], [], []
    ...
    #         # 选择最大簇（点数最多的簇）
    #         main_cluster_label = label_counts.argmax()
    #         main_indices = [i for i, lbl in enumerate(labels) if lbl == main_cluster_label]
    #         if len(main_indices) == 0:
    #             print("[信息] 主簇为空，未提取有效点")
    #             return [], [], [], []
    #         pcd = pcd.select_by_index(main_indices)
    #         print(f"[信息] DBSCAN聚类后点云数量: {len(pcd.points)}")
    #     except Exception as e:
    ...
    #         filtered_x = filtered_points[:, 0].tolist()
    #         filtered_y = filtered_points[:, 1].tolist()
    #         filtered_z = filtered_points[:, 2].tolist()

    #         # 使用原始的索引来筛选对应的颜色
    #         valid_indices = np.array(main_indices)
    #         if valid_indices.size > len(rgb):
    #             print(f"[警告] 有效索引数量大于原始颜色数量，修正索引大小")
    #             valid_indices = valid_indices[:len(rgb)]
    #         filtered_rgb = np.array(rgb)[valid_indices].tolist()

    #         print(f"[信息] 过滤后点云数量: {len(filtered_x)}")
    #         return filtered_x, filtered_y, filtered_z, filtered_rgb

    #     except Exception as e:
    #         print(f"[错误] 提取过滤后点云数据失败: {e}")
    #         return [], [], [], []
    ...
    def create_semantic_object(
        ...
        # 计算执行时间（秒）
        point_loop_time = (end_time - start_time).to_sec()
        # rospy.loginfo(f"point loop time: {point_loop_time:.3f} seconds")
        ...
        # 计算执行时间（秒）
        point_clouds_filter_time = (end_time - start_time).to_sec()
        # rospy.loginfo(f"point clouds filter time: {point_clouds_filter_time:.3f} seconds")
    ...
    def timer_callback(self, event):
        ...
        try:
            ...
            # 计算执行时间（秒）
            detection_time = (end_time - start_time).to_sec()
            rospy.loginfo(" ")
            rospy.loginfo(f"Detection time: {detection_time:.3f} seconds")
            ...
            if len(masks) > 0:
                ...
                # 计算执行时间（秒）
                create_semantic_object_time = (end_time - start_time).to_sec()
                rospy.loginfo(f"create semantic object time: {create_semantic_object_time:.3f} seconds")
