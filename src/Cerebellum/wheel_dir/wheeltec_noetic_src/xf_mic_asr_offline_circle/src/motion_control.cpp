...
#include <xf_mic_asr_offline_circle/Set_Major_Mic_srv.h>
#include <xf_mic_asr_offline_circle/position.h>
#include <string.h>
#include <math.h>
#include <joint.h>
#include <iostream>
...
返回  值：无
**************************************************************************/
void current_position_Callback(const xf_mic_asr_offline_circle::position& msg)
{
...
void goal_reach_Callback(const move_base_msgs::MoveBaseActionResult& msg)
{
    ...
    {
        OTHER = (char*) "/feedback_voice/reach_goal.wav";//语音反馈
    ...
    {
        OTHER = (char*) "/feedback_voice/overmap.wav";//语音反馈
        WHOLE = join((head + audio_path),OTHER);
        system(WHOLE);
        overmap_flag=0;
    }
...
void akm_follow_turn(int angle)
{
    ...
    //goal_angle_r= (angle-direction) /180*3.1415926;
...
int main(int argc, char** argv)
{
  ...
  /***创建障碍物方位话题订阅者***/
    ...
    node.param("/command_recognition/audio_path", audio_path, std::string("~/catkin_ws1111/src/xf_mic_asr_offline/feedback_voice"));
    // if (if_akm=="yes")
    //  i=1;
    // else i=0;
            ...
            if(distance_judgment() && dis_angleX_judgment())    //判断障碍物的距离和方向
            {
                ...
                if(temp_count > radar_count)    //连续计数5️次停止运动防止碰撞，避免雷达有噪点
                {
                    ...
                    else
                    {
                        ...
                        turn_fin_flag = 0;
                        temp_count = 0;
                        OTHER = (char*) "/feedback_voice/find.wav";
                        WHOLE = join((head + audio_path),OTHER);
                        system(WHOLE);
                    }
                }
