...
//设置自动回充状态服务
bool turn_on_robot::Set_Charge_Callback(turtlesim::Spawn::Request &req,turtlesim::Spawn::Response &res)
{
    ...
    if(round(req.x)==1)
      ...
      Send_Data.tx[1] = 0,AutoRecharge=0;//取消自动回充时,同时取消cmd_vel里的标志位状态
    ...
    if( Send_Data.tx[1]==0 )
    {
      if(charge_set_state==0)//等待底盘回传的回充状态再判断是否调用服务成功,形成闭环
    ...
    {
      if(charge_set_state==1)//等待底盘回传的回充状态再判断是否调用服务成功,形成闭环
        res.name = "true";
      else
        res.name = "false";
    }
...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  uint8_t i=0,check=0,check2=0, error=1,error2=1,Receive_Data_Pr[1]; //Temporary variable to save the data of the lower machine //临时变量，保存下位机数据
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("car_mode",         robot_type,    "mini_mec"); //机器人型号
  ...
  RED_publisher     = n.advertise<std_msgs::UInt8>("robot_red_flag", 20); //Create a topic publisher that finds the infrared signal flag bit //创建寻找到红外信号标志位话题发布者
  SetCharge_Service = n.advertiseService("set_charge", &turn_on_robot::Set_Charge_Callback,this);
