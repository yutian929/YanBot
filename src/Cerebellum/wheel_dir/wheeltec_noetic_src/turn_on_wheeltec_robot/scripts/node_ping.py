...
class Node_Detecion:
    ...
    def stopMoving(self):
        velocity = Twist()
        velocity.linear = Vector3(0.0, 0.0, 0.0)
        velocity.angular = Vector3(0.0, 0.0, 0.0)
                ...
                try:
                    ...
                    try:
                        ...
                        if errnum == -2:  # name/service unknown
                            ...
                            print(
                                "ERROR: Unknown host [%s] for node [%s]"
                                % (p.hostname, node_name)
                            )
                        elif errnum == errno.ECONNREFUSED:
                            # check if node url has changed
                            new_node_api = self.get_api_uri(
                                master, node_name, skip_cache=True
                            )
                            if not new_node_api:
                            ...
                            if new_node_api != node_api:
                                if details:
                                    print(
                                        "node url has changed from [%s] to [%s], retrying to ping"
    ...
    def feedback(self):
        while not rospy.is_shutdown():
            ping = check_.rosnode_ping(
                self.node_name, max_count=3, details=False
            )  # max_count为允许ping通次数
