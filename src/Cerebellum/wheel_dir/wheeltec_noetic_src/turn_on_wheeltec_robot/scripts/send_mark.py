...
def pose_callback(msg):
    ...
    if msg.status.status == 3 and count > 0:  # 成功到达任意目标点，前往下一目标点
        ...
        if index == count:  # 当index等于count时，表示所有目标点完成，重新开始巡航
            ...
            print(
                "x:"
                + str(markerArray.markers[index - 1].pose.position.x)
                + ", y:"
                + str(markerArray.markers[index - 1].pose.position.y)
                + ", z:"
                + str(markerArray.markers[index - 1].pose.orientation.z)
                + ", w:"
                + str(markerArray.markers[index - 1].pose.orientation.w)
            )
        ...
        rospy.logwarn(
            "Can not reach the target point "
            + str(index - 1)
            + ":"
            + "\r\n"
            + "x:"
        ...
        # 如果已经尝试过前往尚未抵达的目标点，则前往下一个目标点
            ...
            rospy.logwarn(
                "try reach the target point "
                + str(index - 1)
                + " failed! reach next point:"
                + "\r\n"
                + "x:"
                + str(markerArray.markers[index - 1].pose.position.x)
                + ", y:"
...
def click_callback(msg):
    ...
    print("Add a new target point " + str(count) + ":")
    print("x:" + str(msg.point.x) + ", y:" + str(msg.point.y) + ", z:0" + ", w:1")
...
def send_mark():
    ...
    navGoal_sub = rospy.Subscriber(
        "/send_mark_goal", PoseStamped, navGoal_callback
    )  # 订阅rviz内标记按下的位置
    ...
    goal_status_sub = rospy.Subscriber(
        "/move_base/result", MoveBaseActionResult, pose_callback
    )  # 用于订阅是否到达目标点状态
